    ...
    def getExpression(self, ctx):
        ...
        #8TH GEN: For a complex predicate you must go down to the 8th gen to get the escape clause
            ...
            for i in range(1,ctx.getChildCount(),2):
                for j in range (0, ctx.getChild(i).getChild(0).getChildCount(),2):
                    if temp_relation == None:
                        temp_relation = ctx.getChild(i).getChild(0).getChild((j+1)).getText()
                    if temp_col1 == None:
                        temp_col1 = ctx.getChild(i).getChild(0).getChild(j).getText()
                    else:
                        temp_col2 = ctx.getChild(i).getChild(0).getChild(j).getText()
